#![no_std]
use avr_progmem::{progmem, raw::read_byte};
...
pub fn get_char_bitmap(c: &char) -> [u8; 5] {
    let mut a: [u8; 5] = [0; 5];
    let index: usize = ((*c as usize) - 0x20) * 5;

    let base_ptr = FONT.as_ptr() as *const u8;

    for (i, v) in a.iter_mut().enumerate() {
        unsafe { *v = read_byte(base_ptr.add(index + i)) }
    }
    a
}
progmem! {
pub static progmem FONT:[u8;97*5]=[
